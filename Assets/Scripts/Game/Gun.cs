...
public class Gun : MonoBehaviour
{
    ...
    /// </summary>
    public float shotgunSpread = 45.0f;
    ...
    public void ShootGun(Transform director)
    {
        ...
        if (shotgun) {
            float angleStep = shotgunSpread / shotgunBullets;
            float startAngle = (-angleStep) * shotgunBullets / 2;
            float angle = startAngle;
            for (int i = 0; i < shotgunBullets; i++) {
                SpawnBullet(
                    director.position, 
                    director.rotation * Quaternion.Euler(0, 0, angle) 
                );
                angle += angleStep;
            }
